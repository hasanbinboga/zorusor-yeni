...
namespace ZoruSor.Lib.Test.KuraliBul
{
    ...
    [HighlightedClass]
    public class KurBulUy112Test : BaseTest
    {
        private const int SayfadakiSoruAdet = 4;
        private const int CeldiriciAdet = 5;
        private const int ResimBoyut = 270;
        [HighlightedMember]
        public KurBulUy112Test(Havuz.Havuz havuz, int zorlukDerece, int sabitParcaAdet, int sayfaAdet)
        {
            for (int i = 0; i < SayfadakiSoruAdet * sayfaAdet; i++)
            {
                var soruCreater = new SoruCreater();
                SoruBuilder builder = new KuraliBulUygula11
                {
                    Havuz = havuz,
        ...
        public KurBulUy112Test(IEnumerable<TestDetail> testDetails)
        {
