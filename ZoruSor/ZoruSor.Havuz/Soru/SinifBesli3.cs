...
{
    public class SinifBesli3 : SoruBuilder
    {
        internal static Dictionary<string, int> OrtakParcaList;
        internal int OrtakResimAdet;
        ...
        {
            OrtakParcaList = new Dictionary<string, int>(10);
            ...
            //Kac ortak resim olacagini 3 ile 5 arasinda random olarak belirle
            OrtakResimAdet = RandomHelper.RandomNumber(3, 5);

            //Referans resimden, belirlenen parcalari degisik ortak resim adedi - 1 kadar resim uret.
            for (int i = 0; i < OrtakResimAdet - 1; i++)
            {
            ...
            //Eger ortak resim adedi 5 ten kucuk esitse
            if (OrtakResimAdet <= 5)
            {
                ...
                {
                    if (Soru.ReferansResimList.Take(OrtakResimAdet).Count(s => s.ParcaList[referansParca.Key] == referansParca.Value) >= 3)
                    {
                        OrtakParcaList.Add(referansParca.Key, referansParca.Value);
                    }
                }
                if (OrtakParcaList.Count == 0)
                {
                    throw new ApplicationException("Ortak parca bulunamadi...");
                }
                
                //Rasgele 5-ortak resim adet kadar  resim uret.
                for (int i = 0; i < 5 - OrtakResimAdet; i++)
                {
                ...
                for (int i = OrtakResimAdet; i < 5; i++)
                {

                    Soru.ReferansResimList[i] = KotuParcaDegistir1(Soru.ReferansResimList[i], OrtakParcaList);
                ...
                foreach (var resim in Soru.ReferansResimList)
                {
                    ...
                    {
                        if (OrtakParcaList.ContainsKey(parcaDeger.Key) == false)
                        {
        ...
        public override void CeldiriciUret()
        {
            ...
            for (var i = 0; i < CeldiriciAdet; i++)
            {
                ...
                sonuc = KotuParcaDegistir1(sonuc, OrtakParcaList);
