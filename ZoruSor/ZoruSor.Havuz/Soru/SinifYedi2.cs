...
{
    public class SinifYedi2 : SoruBuilder
    {
        ...
        public override void ReferansResimUret()
        {
            ...
            var resim4 = ResimHelper.ResimDegistirUret(Havuz, resim1, degisecekParcalar, ResimBoyut);
            var resim5 = ResimHelper.ResimDegistirUret(Havuz, resim1, degisecekParcalar, ResimBoyut);

            Soru.ReferansResimList.Add(resim1);
            Soru.ReferansResimList.Add(resim2);
            Soru.ReferansResimList.Add(resim3);
            Soru.ReferansResimList.Add(resim4);
            Soru.ReferansResimList.Add(resim5);

            //Diger 4 referans resimdeki ortak parca adlari ve id' lerini sec
            _ortakParcaList = new Dictionary<string, int>();
            foreach (var referansParca in Soru.ReferansResimList[0].ParcaList)
            {
                if (Soru.ReferansResimList.Count(s => s.ParcaList[referansParca.Key] == referansParca.Value) == 5)
                {
        ...
        {
            //Ortak parçalardan dogru cevap uret
            Soru.DogruCevapList.Add(ResimHelper.ResimUret(Havuz, _ortakParcaList, ResimBoyut));
        }
        ...
        public override void CeldiriciUret()
        {
            ...
            {
                var degisecekParcalar = ParcaSecimHelper.KadariniSec(Havuz, _ortakParcaList.Count);
                var sonuc = ResimHelper.RasgeleResimUret(Havuz, degisecekParcalar, ResimBoyut);
